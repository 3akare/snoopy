...
def mediapipe_detection(image, model):
    """Processes image using MediaPipe model"""
...
def process_video_and_save_sequences(video_path, action_name, data_path=DATA_PATH, sequence_length=SEQUENCE_LENGTH):
    ...
    with mp_hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.5, static_image_mode=False) as hands_model:
        frame_count = 0
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
    ...
    while frames_processed + sequence_length <= num_total_frames:
        ...
        sequence_data_array = np.array(segment, dtype=np.float32)
